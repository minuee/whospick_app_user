...
const PointBuyHistory = props => {
    const [dataList, setDataList] = useState({list: [], year_list: []});
    const [year, setYear] = useState('');
    const [isModalYearOpend, setIsModalYearOpend] = useState(false);
    const _getPointBuyHistory = async () => {
        try {
            const apiResult = await apiObject.getPointBuyHistory({
                year: year,
            });
            setDataList(apiResult);
            setYear(apiResult.year_list[0]);
        } catch (error) {
            //console.log('🚀 ~ _getPointBuyHistory= ~ error', error);
            logging(error.response?.data, 'payments/pay-list/charge');
...
export default PointBuyHistory;
