...
const PointRefundHistory = props => {
    const [dataList, setDataList] = useState({list: [], year_list: []});
    const [year, setYear] = useState('');
    const [isModalYearOpend, setIsModalYearOpend] = useState(false);
    const _getPointRefundHistory = async () => {
        try {
            const apiResult = await apiObject.getPointRefundHistory({
                year: year,
            });
            setDataList(apiResult);
            setYear(apiResult.year_list[0]);
        } catch (error) {
            //console.log('🚀 ~ _getPointRefundHistory= ~ error', error);
            ...
            <ScrollView>
                <View style={{...styles.viewScrollInner}}>
                    <Text style={{fontSize: scale(12), color: '#707070', marginBottom: scale(15)}}>
                        ...
                        <Text>포인트 반환은 감독이 개인 사정으로 오디션 공고를 조기 마감하게 될 경우 자동반환됩니다.</Text>
                    </Text>
                    {
...
export default PointRefundHistory;
